...
describe(`Testing README.md examples:`, function () {
  ...
  it(`README.md Using options examples work fine`, unmutedCallback(function () {
    ...
    console.log('Lorem', 'ipsum'); // console.log prints as usual with no
    // special formatting, that is 'Lorem ipsum \n'
    expect(muter.getLogs()).to.equal(chalk.magenta('Lorem • ipsum ▪▪▪'));
  ...
  it(`README.md Overlapping Muters example works fine`,
    unmutedCallback(function () {
      ...
      muter1.unmute(); // Unmutes console.log but not console.warn (still muted
      // by muter2), now being in an inconsistent state
      muter2.unmute(); // Unmutes console.warn and console.error, putting back
      // muter1 in a consistent state
    })
  );
  ...
  it(`README.md Coordinated muting/capturing example works fine`,
    unmutedCallback(function () {
      ...
      muter.mute(); // The Muter mutes simultaneously console.log, console.warn
      // and console.error
      ...
      muter.unmute(); // The Muter unmutes simultaneously console.log,
  ...
  it(`README.md 'muted' and 'captured' Convenience wrappers example works fine`,
    unmutedCallback(function () {
      ...
      const res1 = safelyMutedFunc('lorem', 'ipsum',
        'dolor', 'sit', 'amet'); // Prints nothing: muter is muting
      const res2 = safelyCapturedFunc('lorem', 'ipsum',
        'dolor', 'sit', 'amet'); // Prints 'lorem\nipsum\ndolor\n';

      expect(res1).to.equal(res2);
      expect(res2).to.equal('lorem\nipsum\ndolor\n'); // muter was capturing
      expect(res2).not.to.equal(muter.getLogs()); // muter is no longer muting
      // nor capturing
  ...
  it(`README.md Handling hidden logging methods example works fine`,
    unmutedCallback(function () {
      ...
      muter.mute(); // Mutes console.info and console.log, therefore muting the
      // custom logging function 'log'
      ...
      muter.unmute(); // Unmutes console.info and console.log, therefore
      // unmuting  the custom logging function 'log'
    })
  );
